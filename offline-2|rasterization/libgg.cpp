...
int get_top_index(Triangle _triangle);
int get_bottom_index(Triangle _triangle);
int get_left_index(Triangle _triangle);
int get_right_index(Triangle _triangle);
void z_buffer_algorithm();
void apply_transformations();
bitmap_image generate_image(string _filename);
...
{
  assert(_v1.size() == 3 && _v2.size() == 3 && _v3.size() == 3);
  Triangle triangle = Triangle(_v1, _v2, _v3);
  triangle = triangle.transform(model_matrix);
  triangles[0].push_back(triangle);
}
...
void ggSaveImage(string _filename)
{
  ...
  z_buffer_algorithm();
  bitmap_image image = generate_image(_filename);
...
int get_top_index(Triangle _triangle)
{
  ...
  for (int i = 1; i < 3; i++)
  {
    if (_triangle.vertices[i].y > _triangle.vertices[top_index].y)
    {
      top_index = i;
    }
  }
...
int get_bottom_index(Triangle _triangle)
{
  ...
  for (int i = 1; i < 3; i++)
  {
    if (_triangle.vertices[i].y < _triangle.vertices[bottom_index].y)
    {
      bottom_index = i;
    }
  }
...
int get_left_index(Triangle _triangle)
{
  ...
  for (int i = 1; i < 3; i++)
  {
    if (_triangle.vertices[i].x < _triangle.vertices[left_index].x)
    {
      left_index = i;
    }
  }
...
int get_right_index(Triangle _triangle)
{
  ...
  for (int i = 1; i < 3; i++)
  {
    if (_triangle.vertices[i].x > _triangle.vertices[right_index].x)
    {
      right_index = i;
    }
  }
...
bitmap_image generate_image(string _filename)
{
  ...
  for (int y = 0; y < screen_height; y++)
  {
    for (int x = 0; x < screen_width; x++)
    {
      image.set_pixel(x, y, color_buffer[x][y].red, color_buffer[x][y].green, color_buffer[x][y].blue);
    }
  }
  return image;
}
